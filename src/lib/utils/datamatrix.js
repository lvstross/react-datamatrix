...
export default function DATAMatrix(Q) {
    var M = [], xx = 0, yy = 0, bit = function (x, y) {
        M[y] = M[y] || [],
            M[y][x] = 1;
    }, toAscii = function (t) {
        var r = [], l = t.length;
        for (var i = 0; i < l; i++) {
            var c = t.charCodeAt(i), c1 = (i + 1 < l) ? t.charCodeAt(i + 1) : 0;
        ...
        var r = [231], /* switch to Base 256 */ l = t.length;
        if (250 < l) {
        ...
        var n = t.length, l = (n + 1) & -4, cw = 0, ch, r = (l > 0) ? [240] : []; /* switch to Edifact */
        for (var i = 0; i < l; i++) {
            if (i < l - 1) {
                /* encode char */
                ch = t.charCodeAt(i);
        ...
        var i, j, cc = 0, cw = 0, l = t.length, r = [s[0]], /* start switch */ push = function (v) {
    ...
    , encodeMsg = function (text, rct) {
        ...
        var M = [];
        var enc = toAscii(text), el = enc.length, 
        // @ts-ignore
        k = toText(text, [
            230,
            31, 0, 0,
        ...
        if (l > 0 && l < el)
            enc = k, el = l;
        var h, w, nc = 1, nr = 1, fw, fh, /* symbol size, regions, region size */ i, j = -1, c, r, s, b = 1, /* compute symbol size */ rs = new Array(70), /* reed / solomon code */ rc = new Array(70), lg = new Array(256), /* log / exp table for multiplication */ ex = new Array(255);
        if (rct && el < 50) {
            /* rect */
            // @ts-ignore
            k = [
    ...
    return (function () {
        ...
        var abs = Math.abs, r, x, y, d, sx, sy, ns = 'http://www.w3.org/2000/svg', path = '', q = ('string' == typeof Q) ? { msg: Q } : Q || {}, p = q.pal || ['#000'], 
        // @ts-ignore
        dm = abs(q.dim) || 256, 
        // @ts-ignore
        pd = abs(q.pad), pd = (pd > -1) ? pd : 2, mx = [1, 0, 0, 1, pd, pd], fg = p[0], fg = ishex(fg) ? fg : '#000', 
        // @ts-ignore
